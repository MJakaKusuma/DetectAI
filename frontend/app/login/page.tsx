...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const data = await apiRequest<LoginResponse>("/login", "POST", { username, password });
      
      // Simpan credentials ke dalam localStorage
      ...
      showToast((err as Error).message || "Gagal masuk. Periksa kembali username dan password anda.", "error");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-[80vh] flex items-center justify-center px-4 py-16 bg-gradient-to-b from-indigo-50/20 to-transparent">
      <div className="max-w-md w-full bg-white border border-slate-100 shadow-2xl shadow-slate-200/80 rounded-2xl p-8 sm:p-10">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-xs text-slate-500 mt-1.5">Gunakan akun anda untuk melakukan analisis mendalam</p>
        </div>
